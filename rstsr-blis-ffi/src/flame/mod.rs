...
pub const LIB_NAME: &str = "FLAME"; // for code, e.g. "MKL"
pub const LIB_NAME_SHOW: &str = "FLAME"; // for display, e.g. "oneMKL"
...
mod dynamic_loading_specific {
    ...
    fn get_lib_candidates() -> Vec<String> {
        ...
        candidates.extend(vec![
            format!("{DLL_PREFIX}flame{BLIS_INT_TYPE_SIZE}{DLL_SUFFIX}"),
            format!("{DLL_PREFIX}flame_{BLIS_INT_TYPE_SIZE}{DLL_SUFFIX}"),
            format!("{DLL_PREFIX}flame-{BLIS_INT_TYPE_SIZE}{DLL_SUFFIX}"),
            format!("{DLL_PREFIX}flame_{BLIS_INT_TYPE_SIZE}{DLL_SUFFIX}"),
            format!("{DLL_PREFIX}flame-{BLIS_INT_TYPE_SIZE}{DLL_SUFFIX}"),
            format!("{DLL_PREFIX}flame{DLL_SUFFIX}"),
        ]);
        candidates
    }

    fn check_lib_loaded(lib: &DyLoadLib) -> bool {
        lib.FLA_Check_num_threads.is_some()
    }
...
Condition: functions `FLA_Check_num_threads` must be present in the library.
