...
pub const LIB_NAME: &str = "BLIS"; // for code, e.g. "MKL"
pub const LIB_NAME_SHOW: &str = "BLIS"; // for display, e.g. "oneMKL"
pub const LIB_NAME_LINK: &str = "blis"; // for linking, e.g. "mkl_rt"
...
mod dynamic_loading_specific {
    ...
    fn check_lib_loaded(lib: &DyLoadLib) -> bool {
        lib.bli_thread_set_num_threads.is_some()
            && lib.dgemm_.is_some()
...
Condition: functions `bli_thread_set_num_threads`, `dgemm_`, and `cblas_dgemm` must be present in the library.
