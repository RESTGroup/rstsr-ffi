...
pub const LIB_NAME: &str = "KML_SERVICE"; // for code, e.g. "MKL"
pub const LIB_NAME_SHOW: &str = "KML service"; // for display, e.g. "oneMKL"
...
mod dynamic_loading_specific {
    ...
    fn get_lib_candidates() -> Vec<String> {
        ...
        candidates.extend(vec![
            ...
            format!("{DLL_PREFIX}kservice{DLL_SUFFIX}"),
        ]);
        candidates
    }

    fn check_lib_loaded(lib: &DyLoadLib) -> bool {
        lib.KmlSetNumThreads.is_some()
    }
...
Condition: `KmlSetNumThreads` function must exist in the library.
