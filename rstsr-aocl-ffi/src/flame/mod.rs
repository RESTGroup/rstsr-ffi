...
pub const LIB_NAME_SHOW: &str = "AOCL FLAME"; // for display, e.g. "oneMKL"
...
mod dynamic_loading_specific {
    ...
    fn check_lib_loaded(lib: &DyLoadLib) -> bool {
        lib.FLA_Check_num_threads.is_some() && lib.dsyevd_.is_some()
    }
...
Condition: `FLA_Check_num_threads` and `dsyevd_` functions must be present.
